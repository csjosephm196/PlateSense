...
// GET existing plan
router.get('/exercise-plan', authMiddleware, async (req, res) => {
    try {
        const plan = await ExercisePlan.findOne({ user_id: req.user._id });
...
router.put('/exercise-plan', authMiddleware, async (req, res) => {
    try {
        const plan = await ExercisePlan.findOneAndUpdate(
